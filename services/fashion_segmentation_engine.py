...
from config import FASHION_DETECTION_DIR, FASHION_CHECKPOINT_PATH, FASHION_LABEL_MAP_PATH, FASHION_CONFIG_FILE, FASHION_INFERENCE_SCRIPT, TPU_DIR
from utils.image_processing import encode_mask_rgba_base64
from utils.session_handler import set_detected_parts

PART_IDS_BLENDING = {28, 29, 30, 31, 32, 33, 34}
UPPER_BODY_IDS = {1, 2, 3, 4, 5, 6, 10, 11, 12, 13}

PART_LABELS = {
    1: "shirt", 2: "t-shirt", 3: "sweater", 4: "cardigan",
    5: "jacket", 6: "vest", 10: "dress", 11: "jumpsuit",
    12: "suit", 13: "coat", 28: "hood", 29: "collar",
    30: "lapel", 31: "epaulette", 32: "sleeve", 33: "pocket",
    34: "neckline",
}

PART_COLORS = {
    "shirt": [128, 128, 128, 128], "t-shirt": [100, 150, 200, 128],
    "sweater": [200, 150, 100, 128], "cardigan": [150, 200, 100, 128],
    "jacket": [200, 100, 150, 128], "vest": [150, 100, 200, 128],
    "dress": [100, 200, 150, 128], "jumpsuit": [250, 150, 50, 128],
    "suit": [50, 150, 250, 128], "coat": [150, 250, 50, 128],
    "sleeve": [255, 80, 80, 128], "collar": [80, 160, 255, 128],
    "lapel": [80, 200, 80, 128], "hood": [255, 180, 50, 128],
    "pocket": [180, 80, 255, 128], "neckline": [255, 255, 80, 128],
...
def run_fashion_segmentation(image_path: Path, output_npy: Path, output_html: Path) -> subprocess.CompletedProcess:
    cmd = [
        sys.executable,
...
def ambil_model_busana(raw_result: Dict) -> Optional[Tuple[np.ndarray, int, int]]:
    """
    Membaca hasil inference Fashionpedia dan memilih SATU label upper body
    dengan jumlah komposisi piksel terbesar.
    (Implementasi sesuai skripsi — diadaptasi dari npy path ke raw_result dict)

    Args:
        raw_result: dict hasil load_segmentation_result() — berisi 'classes', 'masks', 'boxes', 'scores'

    Returns:
        (final_mask, label_upper_body, idx_upper_body) atau None jika tidak ada label cocok.
        - final_mask      : np.array (H,W) uint8 — binary mask label terpilih
        - label_upper_body: int — class id Fashionpedia dari label terpilih
        - idx_upper_body  : int — index di array inference (untuk akses boxes jika diperlukan)
    """
    ...
    for idx, class_label in enumerate(classes):
        ...
        # Pastikan 'counts' dalam format string/UTF-8 sebelum di-decode (sesuai skripsi)
        ...
        # Filter komposisi piksel terbesar (sesuai skripsi)
...
def build_parts_response(result: Dict, session_id: Optional[str] = None) -> Dict:
    ...
    for index in range(num_instances):
        ...
        if class_id in UPPER_BODY_IDS:
            ...
            parts[part_name].append({
                "index": part_idx, "bbox": None, "mask_b64": None,
                "area": 0, "score": round(score, 3), "original_mask": mask
            })
            ...
            parts[part_name].append({
                ...
                "mask_b64": encode_mask_rgba_base64(mask, PART_COLORS.get(part_name, [255, 255, 255, 128])),
    ...
    for part_name, part_idx, ub_mask in upper_body_instances:
        ...
        for p_name, masks_list in part_masks.items():
        ...
        parts[part_name][part_idx]["mask_b64"] = encode_mask_rgba_base64(final_ub_mask, PART_COLORS.get(part_name, [128, 128, 128, 128]))
