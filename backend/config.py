...
MODEL_DIR  = os.getenv("MODEL_DIR", os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "unet")))

# Base model Stable Diffusion (Menggunakan folder lokal jika tersedia untuk meminimalkan unduhan saat startup/deploy)
local_base = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "stable-diffusion-v1-5"))
if os.path.exists(local_base) and os.path.isdir(local_base):
    BASE_MODEL = os.getenv("BASE_MODEL", local_base)
else:
