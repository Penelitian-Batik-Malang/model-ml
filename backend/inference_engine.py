...
class BatikColorizer:
    ...
    def _load_models(self):
        ...
        self.text_encoder = CLIPTextModel.from_pretrained(BASE_MODEL, subfolder="text_encoder", torch_dtype=self.dtype).to(self.device)
        self.vae = AutoencoderKL.from_pretrained(BASE_MODEL, subfolder="vae", torch_dtype=self.dtype).to(self.device)
        self.unet = UNet2DConditionModel.from_pretrained(MODEL_DIR, torch_dtype=self.dtype).to(self.device)
        
        self.vae.eval()
        self.text_encoder.eval()
        self.unet.eval()
        self._is_loaded = True
        print("[FineTuned] Model berhasil dimuat (LOCAL)")
