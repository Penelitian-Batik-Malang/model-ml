    ...
    with tf.variable_scope('mask_head'):
      ...
      with tf.name_scope('masks_post_processing'):
        if self._class_agnostic_mask_pred:
          ...
          # Workaround TF1.15: batch_dims=2 not supported, manual equivalent
          mask_shape = tf.shape(mask_outputs)
          batch_size = mask_shape[0]
          num_rois = mask_shape[1]
          mask_h = mask_shape[2]
          mask_w = mask_shape[3]
          class_indices_int = tf.cast(class_indices, tf.int32)
          batch_idx = tf.tile(
              tf.expand_dims(tf.range(batch_size), 1), [1, num_rois])
          roi_idx = tf.tile(
              tf.expand_dims(tf.range(num_rois), 0), [batch_size, 1])
          gather_nd_idx = tf.stack(
              [batch_idx, roi_idx, class_indices_int], axis=-1)
          mask_outputs = tf.transpose(mask_outputs, [0, 1, 4, 2, 3])
          mask_outputs = tf.gather_nd(mask_outputs, gather_nd_idx)
    return mask_outputs
